...
//								printf("%s \r\n",recvBuf);
				...
				case 2://发送
					{
						/* copy data to pbuf */
						p=pbuf_alloc(PBUF_TRANSPORT,strlen((char*)UdpsendBuf),PBUF_POOL); //申请内存
						if(p)
						{
							pbuf_take(p,(char*)UdpsendBuf,strlen((char*)UdpsendBuf)); //将tcp_demo_sendbuf中的数据打包进pbuf结构中
							printf("Send Message: %s \r\n",UdpsendBuf);
							udp_send(upcb,p);	//udp发送数据 
							pbuf_free(p);//释放内存
						}
					}
					break;
